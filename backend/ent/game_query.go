...
func (gq *GameQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Game, error) {
	...
	if query := gq.withPlayers; query != nil {
		if err := gq.loadPlayers(ctx, query, nodes,
...
func (gq *GameQuery) loadPlayers(ctx context.Context, query *PlayerQuery, nodes []*Game, init func(*Game), assign func(*Game, *Player)) error {
	...
	for _, n := range neighbors {
		fk := n.player_parent
		if fk == nil {
			return fmt.Errorf(`foreign-key "player_parent" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "player_parent" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
