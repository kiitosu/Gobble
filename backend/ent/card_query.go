...
func (cq *CardQuery) QueryItems() *ItemQuery {
...
func (cq *CardQuery) WithItems(opts ...func(*ItemQuery)) *CardQuery {
	query := (&ItemClient{config: cq.config}).Query()
...
func (cq *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := cq.withItems; query != nil {
		if err := cq.loadItems(ctx, query, nodes,
			func(n *Card) { n.Edges.Items = []*Item{} },
			func(n *Card, e *Item) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CardQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*Card, init func(*Card), assign func(*Card, *Item)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(card.ItemsTable)
		s.Join(joinT).On(s.C(item.FieldID), joinT.C(card.ItemsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(card.ItemsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(card.ItemsPrimaryKey[1]))
