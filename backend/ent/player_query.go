...
func (pq *PlayerQuery) QueryParent() *GameQuery {
...
func (pq *PlayerQuery) WithParent(opts ...func(*GameQuery)) *PlayerQuery {
	query := (&GameClient{config: pq.config}).Query()
...
func (pq *PlayerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Player, error) {
	...
	if query := pq.withParent; query != nil {
		if err := pq.loadParent(ctx, query, nodes, nil,
			func(n *Player, e *Game) { n.Edges.Parent = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PlayerQuery) loadParent(ctx context.Context, query *GameQuery, nodes []*Player, init func(*Player), assign func(*Player, *Game)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Player)
	for i := range nodes {
		if nodes[i].player_parent == nil {
			continue
		}
		fk := *nodes[i].player_parent
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "player_parent" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
